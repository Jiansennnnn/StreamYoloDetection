...
from model_nn import AutoBackend
#from utils.augmentations import letterbox
#from utils.general import non_max_suppression, scale_coords, xyxy2xywh, check_img_size
#from utils.plots import Annotator, colors
...
#
        ...
        self.model = AutoBackend(self.weights, device=self.device, dnn=self.dnn, fp16=self.half)
...
#
    ...
    def detect(self, image, show=False):
        ...
        # 包装 pred 并附加 device 信息
        pred = TensorWithDevice(pred, torch.device('cpu'))
        print(f"Before NMS, pred device: {pred.device if isinstance(pred, torch.Tensor) else 'Not a tensor'}")
